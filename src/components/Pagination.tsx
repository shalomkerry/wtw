...
return (
 ...
 <div className="flex items-center justify-center gap-2 my-6 w-2xs bg-neutral-800 px-4 py-2 rounded-lg">
  <button
    ...
    disabled={page === 1}
    className="px-3 py-1 flex  gap-2 items-center justify-center rounded bg-gray-700 hover:bg-gray-600 disabled:opacity-40"
  >
     <img src={backdim} alt="" /> <p>Back</p>
  </button>
{
  Array.from({length:paginationLength}).map((_,i)=>{
    ...
    return (
      ...
      className = {`w-8 h-8 rounded ${page===pageNum?"bg-black text-white":"bg-gray-200"}`}
      >
        {pageNum}
      </button>
    )
